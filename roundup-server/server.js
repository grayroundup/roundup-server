...
app.post("/events/track", async (req, res) => {
  try {
    const { installId, eventType, host, url, amount, charity, timestamp } = req.body || {};
    ...
    const safeAmount = Number.isFinite(amt) ? amt : null;
    ...
    const { error } = await supabase.from("events").insert({
      ...
      amount: safeAmount,
      charity: typeof charity === "string" ? charity : null,
